...
interface BacklogScoperProps {
  ...
  onMarkScopeReady?: (totalHours: number) => void;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 bg-white rounded-2xl border border-[#e2e8f0] shadow-2xs flex flex-wrap items-center justify-between gap-4">
        ...
        {/* Action Buttons: Import from Template, Add Scope & Mark Scope Ready */}
        <div className="flex items-center gap-2">
          {templates.length > 0 && (
            <button
              type="button"
          ...
          {onMarkScopeReady && (
            <button
              type="button"
              onClick={() => onMarkScopeReady(totalHours)}
              disabled={totalHours === 0}
              className="inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-extrabold text-[#0f172a] bg-[#d4ff4a] hover:bg-[#bceb34] disabled:opacity-40 disabled:cursor-not-allowed rounded-xl transition-colors cursor-pointer shadow-xs border border-[#a3e635]"
              title="Notificar a Comercial que el alcance técnico está listo para cotización y pricing"
            >
              <CheckCircle2 className="w-4 h-4 text-[#0f172a]" />
              <span>Marcar alcance listo ({totalHours}h)</span>
            </button>
          )}
          ...
          {onGoToCalculator && (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-bold text-[#475569] bg-white hover:bg-[#f8fafc] border border-[#cbd5e1] rounded-xl transition-colors cursor-pointer shadow-2xs"
              title="Revisar vista de cotización comercial"
            >
              <Calculator className="w-4 h-4 text-[#501f92]" />
              <span>Cotizador</span>
            </button>
          )}
        </div>
      ...
      {/* Handoff Status Notification Banner if technical scoping was marked ready */}
      {opportunityStatus === 'internal_review' && (
        <div className="p-4 rounded-2xl bg-gradient-to-r from-[#ecfdf5] via-[#f0fdf4] to-[#f7fee7] border border-[#a7f3d0] flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs shadow-2xs animate-in fade-in duration-200">
          <div className="flex items-start gap-2.5">
            <div className="w-8 h-8 rounded-xl bg-[#10b981]/15 text-[#047857] flex items-center justify-center shrink-0 mt-0.5">
              <CheckCircle2 className="w-5 h-5 text-[#059669]" />
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <span className="font-extrabold text-[#065f46] text-sm">
                  Handoff Líder → Comercial Completado ({totalHours}h)
                </span>
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#10b981] text-white">
                  Notificado a Comercial
                </span>
              </div>
              <p className="text-[#047857] mt-0.5">
                El líder completó el dimensionamiento técnico ({allBacklogItems.length} actividades en {quote.deliverables.length} entregables). Notificación enviada a Comercial (<strong>{commercialOwner || 'Cata · Directora Comercial'}</strong>) para estructurar pricing, márgenes y condiciones comerciales sin bloquear la navegación del líder.
              </p>
            </div>
          </div>
          {onGoToCalculator && (
            <button
              type="button"
              onClick={onGoToCalculator}
              className="px-3.5 py-2 rounded-xl bg-[#047857] hover:bg-[#065f46] text-white font-bold text-xs transition-colors cursor-pointer shrink-0 shadow-xs flex items-center gap-1.5 self-start sm:self-auto"
            >
              <Calculator className="w-3.5 h-3.5 text-[#d4ff4a]" />
              <span>Ver Cotizador Comercial</span>
            </button>
          )}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 space-y-4">
          {quote.deliverables.length === 0 ? (
            <div className="p-8 text-center bg-white rounded-3xl border-2 border-dashed border-[#cbd5e1] space-y-4">
              <div className="w-14 h-14 rounded-2xl bg-[#501f92]/10 flex items-center justify-center mx-auto text-[#501f92]">
                <Layers className="w-7 h-7" />
              </div>
              <div className="space-y-1">
                <h3 className="text-base font-extrabold text-[#0f172a]">
                  ¿Cómo quieres iniciar el alcance?
                </h3>
                <p className="text-xs text-[#64748b] max-w-md mx-auto">
                  Como líder técnico, puedes partir de una plantilla oficial del catálogo o estructurar un alcance a medida desde cero.
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
                {templates.length > 0 && (
                  <button
                    type="button"
                    onClick={() => setIsImportModalOpen(true)}
                    className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 text-xs font-bold text-[#501f92] bg-[#501f92]/10 hover:bg-[#501f92]/20 border border-[#501f92]/30 rounded-2xl transition-colors cursor-pointer shadow-2xs"
                  >
                    <BookOpen className="w-4 h-4 text-[#501f92]" />
                    <span>Usar plantilla del catálogo</span>
                  </button>
                )}
                <button
                  type="button"
                  onClick={handleAddDeliverable}
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 text-xs font-bold text-white bg-[#501f92] hover:bg-[#3d1572] rounded-2xl transition-colors cursor-pointer shadow-xs"
                >
                  <Plus className="w-4 h-4 text-[#d4ff4a]" />
                  <span>Crear alcance a medida</span>
                </button>
              </div>
            </div>
          ) : (
